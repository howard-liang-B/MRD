...
## {set_class_values、get_label_mask} 被 datasets.py 使用
...
# 此函式被 engine.py 的 validate 驗證時使用
...
def save_plots(
    ...
    plt.plot(
        train_acc, color='tab:blue', linestyle='-',  # tab: 前綴表示這是 Matplotlib 的 tab 色彩系列的一部分。
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()  # 顯示圖例，根據 label 的值來區分不同的曲線。
...
#########################  {predict、draw_segmentation_map、image_overla} 在 infer_image.py 引入  #########################
